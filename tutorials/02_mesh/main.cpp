...
// 1) You probably noticed jagged look of a lines caused by aliasing artifacts: play with frContextSetAASampleCount to get better results
// 2) Play with the camera position and aim to get better view of the cube shadow
// 3) Try to play with the cube matrix to get an annimation rotation
// 4) You might have noticed that the shadow is "hard" which is caused by infinitely small area light. Try to use area light instead to produce nice "soft" shadow.
// 5) There is some noise visible on the cube surface. Try to increase the number of iterations (refinement steps) to get smoother look.
